...
| Deployment | Railway (`ingest`/`consumer`/`api` + self-hosted `redpanda`/`timescaledb`) + Vercel (`apps/web`) | Standalone from Flora's Railway project. `redpanda`/`timescaledb` are self-hosted on Railway since no managed equivalent exists for either (Redpanda has no Railway plugin; Railway's managed Postgres lacks the Timescale extension). Provisioned and deployed via `.railway/railway.ts` (Railway's TypeScript config-as-code) + `railway up` — see `docs/tasks/TASK-railway-deploy.md` for the full topology and the platform-specific issues resolved along the way (docker.redpanda.com unreachable from Railway's build fleet, non-root volume permissions, Docker build-context-vs-Dockerfile-path resolution). `apps/web` moved off Railway to Vercel-only (2026-08-27, task doc §7) once its data fetching became a real server-side prefetch that needs Next's fetch Data Cache/edge network to pay off. |
