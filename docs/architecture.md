...
| Deployment | Railway, own project | Standalone from Flora's Railway project. Six services (`ingest`/`consumer`/`api`/`web` built from this repo's own Dockerfiles; `redpanda`/`timescaledb` self-hosted, no managed equivalent exists for either — Redpanda has no Railway plugin, and Railway's managed Postgres lacks the Timescale extension). Provisioned and deployed via `.railway/railway.ts` (Railway's TypeScript config-as-code) + `railway up` — see `docs/tasks/TASK-railway-deploy.md` for the full topology and the platform-specific issues resolved along the way (docker.redpanda.com unreachable from Railway's build fleet, non-root volume permissions, Docker build-context-vs-Dockerfile-path resolution). |
